...
public partial class Users_Homepageds : System.Web.UI.Page
{
    ...
    void rayon()
    {
        DataTable region2 = klas.getdatatable("select RegionsID,CASE WHEN CityID = 2 THEN Name + ' ' + 'rayonu' WHEN CityID = 1 THEN Name + ' ' + N'şəhərin' END as Name from List_classification_Regions where ForDelete=1 order by Name");
    ...
    protected void selectlivingArea()
    {
...
inner join List_classification_Municipal lcm on t.MunicipalID=lcm.MunicipalID where  t.fordelete=1 and ExitDate is null " + MunicipalId + ray +
"union select '1' sn, " +
